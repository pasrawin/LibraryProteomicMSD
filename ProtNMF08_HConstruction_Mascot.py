...
def h_construction(WithRTPrediction_, WithCosineH_, initRT_tuple, initRT_width,\
	...
	if WithCosineH_:
		...
		if H_mat.shape[0] > H_size_rowcut:
			...
			for row in np.arange(0,H_mat.shape[0],H_size_rowcut):
				print row, timeit.default_timer()
	...
	print ' chk min H_cosmat:', H_cosmat.data.min()
	...
	# sparse.save_npz(str(output_file)+'_Hcosmat_init.npz', H_cosmat)
...
def h_cos(V_mat, W_mat, H_mat, iso_maxnumber, cos_cutoff):
	...
	for k in np.arange(H_matrow): # use H no noise
		...
		H_coslist_k.append(([k],k_count))
		# print '*', v_count
	H_coslist_k = [x*y for (x,y) in H_coslist_k]
	H_coslist_k = [x for sublist in H_coslist_k for x in sublist]
	# print ' H_coslist_k:', len(H_coslist_k)
	# print H_coslist_k
