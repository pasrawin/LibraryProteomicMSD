...
def expmat_construction(exp_file, exp_paramlist, charge_list):
	...
	for index, row in exp_df.iterrows():
		...
		mzint_tuple = [(mz,row['starttime'],value) for (mz,value) in zip(np.arange(MZ_SCALE), timecol_array) if value != 0]
		mzint_tuple_keep.extend(mzint_tuple)
	...
	# expdf_value = [x for sub in time_col for x in sub]

	# expdf_row = np.tile(np.arange(MZ_SCALE), exp_df.shape[0])
	# expdf_col = np.repeat(exp_df['starttime'].values, MZ_SCALE)
	expdf_value, expdf_row, expdf_col = [], [], []
	for row, col, value in mzint_tuple_keep:
		expdf_value.append(value)
		expdf_row.append(row)
		expdf_col.append(col)
	del(mzint_tuple_keep)
