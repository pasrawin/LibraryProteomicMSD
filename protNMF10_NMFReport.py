...
def nmf_savemat(V_mat, W_mat, Hmat, weight, outfile):
	...
	if V_mat != None:
		sparse.save_npz(str(outfile)+'_Vmat_renormalization.npz', V_mat)
	if W_mat != None:
		sparse.save_npz(str(outfile)+'_Wmat_renormalization.npz', W_mat)
	sparse.save_npz(str(outfile)+'_Hmat_renormalization_'+str(round(weight,2))+'.npz', Hmat)
...
def report_all(refresh_H, cos_mat, noiseW_list, noise_mean, shift, tt, gradient_starttime, finalMS1_df, peak_report):
	...
	idenhead_df = finalMS1_df.loc[:,['prot','pept','modpept','mod','charge','mz','isoab0']]
