...
def refMS1_construction(refms1_df, M_header, iso_header, charge_list, iso_maxnumber, globalparam_list, eps):
	...
	mziso_df = mziso_df.reset_index().melt(['prot','pept_id','pept','mod','modpept','rtpeak'] + iso_header, M_header)
	...
	pept_ioncount = mziso_df['pept'].value_counts().sort_index().to_frame(name='ms1count')
	...
	if mziso_df.index.values.tolist() != list(np.arange(len(mziso_df.index.values))):
